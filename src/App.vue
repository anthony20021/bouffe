  ...
  <main class="app-shell">
    ...
    <section v-else class="game-layout">
      ...
      <div class="game-panel">
        ...
        <template v-else-if="game.phase === 'round_result'"><div class="center-state"><p class="eyebrow">RÉSULTAT</p><h2>{{ game.result?.message }}</h2><div class="compare"><div class="reference"><img v-if="game.referenceProduct?.imageUrl" :src="game.referenceProduct.imageUrl" alt="" /><div><small class="eyebrow">Référence</small><strong>{{ game.referenceProduct?.name }}</strong></div></div><div class="reference"><img v-if="game.proposals.at(-1)?.product.imageUrl" :src="game.proposals.at(-1).product.imageUrl" alt="" /><div><small class="eyebrow">Proposition</small><strong>{{ game.proposals.at(-1)?.product.name }}</strong></div></div></div><button class="submit" @click="nextRound">Manche suivante</button></div></template>
